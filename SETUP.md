...
**公告の新規追加**
1. 区分（NEW・空白・結果）、駐屯地、状態を選択
2. 入札日は`R8.8.10`形式で入力
3. 品名、備考を入力
4. PDF表示名、PDFリンク（R8/be/xxx.pdf 形式）を入力
5. PDFファイルを選択（オプション）
6. 「保存」をクリック
...
1. 一覧から「削除」ボタンをクリック
2. 確認ダイアログで「OK」
3. 上部メニューの「削除一覧」で削除内容を確認

修正・削除は登録者も利用できます。出力・取込などの管理機能は、管理者パスワード`snk`で管理者機能を有効化すると利用できます。パスワード欄はEnterキーでも有効化できます。
